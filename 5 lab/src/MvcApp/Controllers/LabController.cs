...
public class LabController : Controller
{
    ...
    public async Task<IActionResult> Third([FromQuery] InputOutputModel inputOutput)
    {
        if (!string.IsNullOrWhiteSpace(inputOutput.InputValue))
        {
            try
            {
                var labThird = new Lab3.Lab();

                var result = await labThird.Execute(new(inputOutput.InputValue));
